...
export default function List({items, setItems, onClick, className, checked, readonly, editInterface = false} : List) {
    ...
    return(
        <Reorder.Group axis="y" values={items} onReorder={setItems} className={className}>
        {items.map((item, i) =>
                <ListItem
                    key={item}
                    item={item}
