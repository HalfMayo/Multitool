...
export const SetterBreak = memo(function SetterBreak({breakLength, dispatch}:BreakProps) {

    return(
        <div className="flex flex-col items-center justify-center bg-surface-container px-4 pt-4 pb-2 rounded-md">
            <h3>Break Length</h3>
            <div className="flex items-center justify-center gap-4">
              <SvgButton svg={Minus} label="decrease break length" onClick={() => dispatch({type:"break/decrement"})}/>
                <h3>{breakLength}</h3>
                <SvgButton svg={Plus} label="increase break length" onClick={()=> dispatch({type:"break/increment"})}/>
            </div>
        </div>
    )
})