        ...
        const newData = friendsList.map((friend) => {
          if (friend.id === selectedFriend) {
            return {
              ...
              remaining:
                friend.remaining +
                (payer === myFriend?.name ? parsedPaid : -parsedOther),
            };
          }
          return friend;
        });
    ...
    <>
      ...
      >
        <div className="w-full">
          <label className="block mb-1" htmlFor="payer">
            Who paid the bill?
          </label>
          <select
        ...
        <div className="w-full autofill-w">
          <label htmlFor="youPaid" className="block mb-1">
            What's your share?
          </label>
          <input
            className="w-full py-1 px-2"
            ...
            id="youPaid"
            value={youPaid}
            onChange={(e) => setYouPaid(e.target.value)}
          ></input>
        </div>
        <div className="w-full flex flex-col items-center justify-center py-2 gap-2">
          ...
          {calc &&
            (payer === myFriend?.name ? (
              <Tip className="w-full">
                ...
                <p>
                  For this bill, you owe {myFriend?.name} {youPaid}€
                </p>
              </Tip>
            ) : payer === "you" ? (
              <Tip className="w-full">
                <Hand width="24px" height="24px" />
                <p>
                  For this bill, {myFriend?.name} owes you {otherPaid}€
                </p>
              </Tip>
            ) : (
              <></>
          ...
          {calc &&
            typeof myFriend?.remaining === "number" &&
            (myFriend?.remaining < 0 ? (
              <Tip className="w-full">
                <Piggie width="24px" height="24px" />
                <p>
                  Overall, {myFriend?.name} still owes you{" "}
                  {Math.abs(myFriend?.remaining)}€
                </p>
              </Tip>
            ) : myFriend?.remaining > 0 ? (
              <Tip className="w-full">
                <Piggie width="24px" height="24px" />
                <p>
                  Overall, you still owe {myFriend?.name} {myFriend?.remaining}€
                </p>
              </Tip>
            ) : (
              <Tip className="w-full">
