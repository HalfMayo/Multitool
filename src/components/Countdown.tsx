...
export const Countdown = memo(function Countdown({sessionTimer, handleReset, handlePlayPause, minutes, seconds, pause, audioRef} : CountdownProps) {
    ...
    <div className="flex flex-col items-center justify-center text-lg font-semibold bg-surface-container w-full p-4 rounded-md">
          ...
          <div className="flex items-center justify-center gap-4 text-xl font-bold">
            <SvgButton label="Reset Timer Settings" onClick={handleReset} svg={Refresh}/>
            <h1>{minutes}:{seconds}</h1>
            <SvgButton label={pause ? "Start Timer" : "Pause Timer"} onClick={handlePlayPause} svg={pause ? Play : Pause}/>
            <audio aria-hidden="true" src="https://actions.google.com/sounds/v1/alarms/bugle_tune.ogg" ref={audioRef}></audio>
          </div>
       </div>)
  })